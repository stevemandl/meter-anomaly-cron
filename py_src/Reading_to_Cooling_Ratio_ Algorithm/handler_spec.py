...
#Temp_decrease_CW_increase.json contains the response from the following JSON request body:

temp_decrease_CW_increase_testcase = open('Reading_to_Cooling_Ratio_ Algorithm/Testcases/Temp_decrease_CW_increase.json')

#Temp_increase_CW_decrease.json contains the response from the following JSON request body:

temp_increase_CW_decrease_testcase = open('Reading_to_Cooling_Ratio_ Algorithm/Testcases/Temp_increase_CW_decrease.json')
...
def test_T_decrease_CW_increase(mocker):
    event = {
        "body": {
            "pointName": "KlarmanHall.Elec.Solar.PowerScout3037/kW_System",
            "timeStamp": "2022-10-05T23:58:47.390Z",
    ...
    data = json.load(temp_decrease_CW_increase_testcase)
    mocker.patch(
        "python_template.handler.fetch_trends",
        return_value = data,
    )
    result = run(event, None)
    assert "statusCode" in result
    assert "Cold water increased when the temperature fell" == result.get("body")
    
def test_T_increase_CW_decrease(mocker):
    event = {
        "body": {
            "pointName": "KlarmanHall.Elec.Solar.PowerScout3037/kW_System",
            "timeStamp": "2022-10-05T23:58:47.390Z",
    ...
    data = json.load(temp_increase_CW_decrease_testcase)
    mocker.patch(
        "python_template.handler.fetch_trends",
        return_value = data,
    )
    result = run(event, None)
    assert "statusCode" in result
    assert "Cold water decreased when the temperature rose" == result.get("body")
